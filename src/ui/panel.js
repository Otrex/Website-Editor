...
export class PropertiesPanel {
  constructor({ root, editor }){
    this.root = root; this.editor = editor;
    this._mount();
  }
  _mount(){
  ...
  renderList(){
    ...
    this.countEl.textContent = `${st.elements.length} element${st.elements.length !== 1 ? 's' : ''}`;
    this.listEl.innerHTML = st.elements.map(el => {
      const text = (el.props?.text && el.props.text.length > 20) ? el.props.text.slice(0,20) + '…' : (el.props?.text || el.id);
    ...
    this.listEl.querySelectorAll('.element-item').forEach(n => {
      n.addEventListener('click', (e)=>{
        if(e.target.matches('[data-del]')){ e.stopPropagation(); this.editor.deleteElement(e.target.dataset.del); }
        else { this.editor.select(n.dataset.id); }
      });
    });
  }
}